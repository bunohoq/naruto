...
| 파일 | 재생 타이밍 | 볼륨 |
|------|------------|------|
| `jutsu_activate.mp3` | 모든 인술 발동 시 | 0.9 |
| `bunshin_voice.mp3` | 분신술 발동 시 (나루토 대사) | 1.0 |
| `bunshin_clone.mp3` | 분신 복제 생성 시 | 0.85 |

- 모든 사운드는 `assets/sounds/` 폴더에 위치
- 같은 사운드가 겹쳐도 처음부터 다시 재생 (`currentTime = 0` 리셋)
- 브라우저 자동재생 정책으로 인해 첫 제스처 인식 전 페이지와 한 번 상호작용 필요
...
- **Web Audio API (HTMLAudioElement)** — 인술 효과음 재생
